 ...
 * Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307 begin_of_the_skype_highlighting            02111-1307      end_of_the_skype_highlighting  USA
 *
 */
...
#define S3C6410_USB_PHYPWR (0x7c100000 + 0)
#define S3C6410_USB_PHYCTRL (0x7c100000 + 4)
#define S3C6410_USB_RSTCON (0x7c100000 + 8)
...
#define printf_S3C_UDC_SETUP
#define printf_S3C_UDC_EP0
#define printf_S3C_UDC_ISR
#define printf_S3C_UDC_OUT_EP
#define printf_S3C_UDC_IN_EP
//#define printf_S3C_UDC
...
#if defined(printf_S3C_UDC_SETUP) || defined(printf_S3C_UDC_ISR)	  \
	|| defined(printf_S3C_UDC_OUT_EP)
...
void otg_phy_init(void)
{
	...
	/* Enable PHY */
#if 0
	writel(readl(S3C6410_USB_PHYCTRL)|(0x1<<0),
	       S3C6410_USB_PHYCTRL);
...
void otg_phy_off(void)
{
	...
	writel(readl(S3C6410_USB_PHYCTRL)&~(1<<0),
	       S3C6410_USB_PHYCTRL);
...
static int udc_enable(struct s3c_udc *dev)
{
	...
	printf("S3C USB 2.0 OTG Controller Core Initialized : 0x%x\n",
		    readl(&reg->gintmsk));
...
static void reconfig_usbd(void)
{
	...
	while (readl(&reg->grstctl) & 0x10)
		printf("%s: waiting for S3C_UDC_OTG_GRSTCTL\n", __func__);
			 ...
			 const struct usb_endpoint_descriptor *desc)
{
	...
	if ((desc->bmAttributes == USB_ENDPOINT_XFER_BULK
		...
		printf("\x1b[1;31m%s: bad %s maxpacket\x1b[0m\n", __func__, _ep->name);
		printf("who cares, but wMaxPacketSize %d, ep_maxpacket(ep) %d\n",le16_to_cpu(desc->wMaxPacketSize),ep_maxpacket(ep));
//		v=1;
//		return -ERANGE;
	}
	...
	if (v==0)
		...
		ep->ep.maxpacket = le16_to_cpu(ep_maxpacket(ep));
...
int s3c_udc_probe(struct s3c_plat_otg_data *pdata)
{
	...
	DEBUG("phy base is 0x%x, otg base is 0x%x\n",phy,reg);
	...
	for (i = 0; i < S3C_MAX_ENDPOINTS+1; i++) {
		v = kmalloc(DMA_BUFFER_SIZE+16, GFP_KERNEL);
		v += 16;
		v &= 0xfffffff0;
		dev->dma_buf[i] = v;
		dev->dma_addr[i] = (dma_addr_t) dev->dma_buf[i];
		DEBUG("\x1b[1;35m%s:\x1b[1;33m dma_buf[%d]=0x%x, dma_addr[%d]=0x%x\x1b[0m\n",__func__,i,dev->dma_buf[i],i,dev->dma_addr[i]);
	}
